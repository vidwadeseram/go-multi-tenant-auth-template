...
BASE_URL="${1:-http://localhost:8006/api/v1}"
PASS=0
FAIL=0
...
echo "============================================"
echo "  Go Multi-Tenant Auth — Curl Test Suite"
...
echo "$ME" | python3 -c "import sys,json; assert json.load(sys.stdin)['data']['user']['email']=='testuser@example.com'" && pass "GET /auth/me" || fail "GET /auth/me" "$ME"

echo ""
echo "--- Auth: Refresh + Forgot Password + Logout ---"
...
CONTAINER=$(docker ps --filter "publish=8006" --format "{{.Names}}" | head -1)
if [ -n "$CONTAINER" ]; then
...
UPDATE=$(curl -s -X PATCH "$BASE_URL/tenants/$TENANT_ID" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"name":"Test Corp Updated"}')
echo "$UPDATE" | python3 -c "import sys,json; assert json.load(sys.stdin)['data']['name']=='Test Corp Updated'" && pass "PATCH /tenants/:id" || fail "PATCH /tenants/:id" "$UPDATE"
...
MEMBER_ROLE_ID=$(docker exec $(docker ps --filter "publish=8006" --format "{{.Names}}" | head -1) psql -U postgres -d authdb -t -c "SELECT id FROM roles WHERE name='tenant_member';" | tr -d ' \n')
